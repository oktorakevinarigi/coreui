              ...
              <b-card-body>
                <b-form-group label="Prepended text" label-for="elementsEmail" description="Here's some help text">
                  <b-input-group>
                    ...
                    <b-form-input id="elementsEmail" name="nama" type="text" autocomplete="email" v-model="nama" v-validate="'required'" v-bind:class="{'form-control': true, 'error': errors.has('nama') }" ></b-form-input>
                ...
                <div class="form-actions">
                  <b-button type="submit" variant="primary" :disabled="errors.any()" v-on:click="reducePrice(4)">Reduce</b-button>
                  <!-- <b-button type="button" variant="secondary">{{nama}}</b-button> -->
                </div>
              </b-card-body>
            </b-collapse>
            </form>
...
export default {
  ...
  methods: {
    // reducePrice: function(data){
    //   this.$store.dispatch('reducePrice', data)
    // }
    // ...mapActions(["reducePrice"]),
    // onSubmit: function() {
    //   this.$validator.validateAll().then((result) => {
    //     debugger
    //       console.log('form is valid', this.nama);
    //         // this.$store.dispatch('onChange', 's')
    //   }).catch(() => {
